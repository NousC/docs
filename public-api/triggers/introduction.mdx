...
A trigger is a subscription. You give Nous a URL and a list of events; whenever one of those events happens on the Account Record, Nous POSTs a signed payload to your URL.
...
| Event | Fired when |
|---|---|
| `interaction.email_received` | A reply lands (Instantly, Smartlead, EmailBison, Lemlist, Gmail) |
...
Triggers are workspace-scoped. Auth works with either the dashboard JWT or an API key, so you can register subscriptions from a script or n8n flow.

<CardGroup cols={2}>
  <Card title="GET /api/triggers" href="/public-api/triggers/list" icon="list">
    List subscriptions in the workspace.
  </Card>
  <Card title="POST /api/triggers" href="/public-api/triggers/create" icon="plus">
    Create a subscription and receive the signing secret.
  </Card>
  <Card title="PATCH /api/triggers/:id" href="/public-api/triggers/update" icon="pen">
    Update name, URL, events, active, or rotate the secret.
  </Card>
  <Card title="DELETE /api/triggers/:id" href="/public-api/triggers/delete" icon="trash">
