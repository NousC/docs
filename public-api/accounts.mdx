...
`recent_observations` is **truncated** to the most recent 50 events. Treat counts derived from it (number of replies, messages, etc.) as lower bounds, not totals. For accurate counts across a longer window or many entities, use [`POST /v2/query`](/public-api/query).
