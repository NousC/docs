...
const { items } = await nous.query(
  { kind: 'event', property: 'interaction.email_replied', since_days: 7, limit: 10 },
  { return: 'entities' },
);
items.forEach(e => console.log(e.entity_name, '—', e.matches, 'replies, last', e.most_recent_at));

// Sent without reply in last 5 days
const noReply = await nous.query(
  { kind: 'event', property: 'interaction.email_sent', since_days: 5 },
  {
    return: 'entities',
    without: { kind: 'event', property: 'interaction.email_replied', since_days: 5 },
  },
...
// Funnel
const funnel = await nous.query(
  { kind: 'state', property: 'stage' },
);
console.log(funnel.rollups.by_value);
```
...
for e in hot["items"]:
    print(e["entity_name"], "—", e["matches"], "replies, last", e["most_recent_at"])

# Sent without reply in last 5 days
no_reply = client.query(
    scope={"kind": "event", "property": "interaction.email_sent",
           "since_days": 5},
    return_="entities",
    without={"kind": "event", "property": "interaction.email_replied",
             "since_days": 5},
)

# Funnel
funnel = client.query(scope={"kind": "state", "property": "stage"})
print(funnel["rollups"]["by_value"])
```
</CodeGroup>
