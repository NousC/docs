...
<Steps>
  ...
  <Step title="Create an API key">
    Click **Add**, give the key a name (e.g. *Proply*), choose a never-expiring or long-lived expiration, and copy the `cal_live_…` token shown.
  </Step>
...
There's no webhook URL to copy. When you save the connection, Proply calls `POST https://api.cal.com/v2/webhooks` and registers the subscription for you. A **Webhook ✓** badge appears on the Cal.com card once it's live.

## What gets logged

Proply subscribes to three Cal.com triggers:
...
New attendees that aren't yet contacts in Proply are **created automatically** on `BOOKING_CREATED`.

## Backfill on CSV import

When you import contacts via CSV, Proply automatically queries Cal.com for each contact's past bookings:
