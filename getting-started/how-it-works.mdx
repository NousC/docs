---
title: "How it works"
description: "Learn the architecture of Nous"
---

## Architecture

Nous is composed of 3 main services and 5 external signal sources — all 3 main services run within a **single Docker container** and talk to each other directly. Signal sources push data in through OAuth and webhooks; the unified record is exposed to agents via MCP.
...
* [Live Sync](#live-sync) - Propagates changes back to connected tools. Update a stage in Nous, it updates in Salesforce.
...
Signal ingestion is how Nous learns about your relationships. It connects to:
...
Identity resolution is what makes Nous a unified layer rather than just another data store. Every inbound signal is matched to a single contact record using:
...
Changes in Nous propagate back to connected tools automatically:
...
The MCP connector is how AI agents read and write to Nous without a UI. Add it to your `mcp.json` and your agent can call tools like `get_contact`, `log_activity`, and `update_pipeline_stage` directly.
