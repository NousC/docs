...
The MCP server gives your agent the Nous context graph as a focused set of tools. Engineered, sourced context, never raw rows. Your agent can set the workspace up itself, so have it call `get_workspace_status` first.

Run it locally with `npx`, or use the hosted HTTP endpoint. Both hit the same graph, so a write through one shows in the other. For known-parameter calls, prefer the [REST API](/public-api/introduction).
...
For clients that cannot launch a local process, like n8n cloud or any remote-URL client. Same tools, same graph. Authenticate each request with your API key as a Bearer token.
