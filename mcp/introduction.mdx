...
The Nous MCP server exposes the v2 customer graph as **seventeen tools** any MCP-compatible client can call. Your agent gets engineered, epistemics-tagged context, never raw rows — and can set the workspace up itself: onboard it, build the GTM playbook, connect key-based tools, configure CRM sync, and wire triggers. Nous is operated by the agent; the web app is where you watch it. Call `get_workspace_status` first to see what's set up and what to do next.
...
`/nous-login` opens your browser, you approve, and a workspace API key is minted and saved automatically — no copy-paste. (Prefer to paste a key? Create one at Settings → API Keys and set it as the plugin's `NOUS_API_KEY`.) Seventeen tools become callable in every Claude Code session.
...
<Note>`get_workspace_facts` is still registered as a back-compat alias of `get_gtm_profile`; new integrations should call `get_gtm_profile`.</Note>
...
The Claude Code plugin's preferred path is `/nous-login` — a browser sign-in that mints a workspace key and saves it for you (no `NOUS_API_KEY` to set). The plugin's `userConfig` field still accepts a pasted key as a fallback, stored in your OS keychain — never in plaintext or shell history.
...
```bash
NOUS_API_KEY=... npx -y @opennous/mcp@0.19.0
```
