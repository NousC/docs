...
<Note>
**Optional: the [Claude Code plugin](https://github.com/NousC/opennous/tree/main/plugins/nous).** Adds hooks that make Claude reach for Nous automatically on GTM work. A power-up, not a requirement — the command above already gives you every tool. `/plugin marketplace add NousC/opennous` then `/plugin install nous@nous-plugins`.
</Note>
...
Source lives at [`apps/mcp`](https://github.com/NousC/opennous/tree/main/apps/mcp); each release ships to npm as `@opennous/mcp@<semver>`. Pin a version for reproducible installs: `npx -y @opennous/mcp@0.43.0`.
