...
<Note>
Also callable: **`record_closed_deals`** (train the ICP model on real closed-won/lost deals — contrastive lift), **`get_routing_preferences`** (Claude Code routing defaults so GTM work defaults to Nous), and the lead tools **`lead_list_operations`** and **`coverage`** (pre-spend dedup / coverage check — cloud-only, Lead Lists).
</Note>
