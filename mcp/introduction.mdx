...
<Note>
Also callable: **`record_closed_deals`** (train the ICP model on real closed-won/lost deals — contrastive lift), **`get_routing_preferences`** (Claude Code routing defaults so GTM work defaults to Nous), and the lead tools **`lead_list_operations`**, **`check_leads`**, **`lead_coverage`** (cloud-only — Lead Lists). Plus `get_workspace_facts`, a back-compat alias of `get_gtm_profile` — new integrations should call `get_gtm_profile`.
</Note>
