...
Turn the recorded GTM context into a weighted set of ICP scoring signals so accounts get scored for fit. This is the second half of building the GTM playbook: first record the ICP and how they sell with [`update_gtm_profile`](/mcp/tools/update-gtm-profile), then call this. If a model already exists it's left alone unless you pass `force: true`. If it reports no GTM context yet, record some first, then call again.
...
- After recording the GTM context with `update_gtm_profile`, to create the scoring model.
