...
Next: if the user can name a few closed-won + closed-lost domains, call record_closed_deals to sharpen it on real outcomes, then call export_icp_model to write the learned model back into their ICP file.
```
...
To write the learned model back into the file, use [`export_icp_model`](/mcp/tools/export-icp-model). To sharpen the model on real outcomes, use [`record_closed_deals`](/mcp/tools/record-closed-deals).
