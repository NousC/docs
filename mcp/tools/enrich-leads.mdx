...
Select leads with `filter` rather than listing ids — `filter` is resolved server-side across the whole list, not just one page. With neither `filter` nor `ids`, it defaults to every lead missing an email (the usual case). Call [`lead_list_operations`](/mcp/tools/lead-list-operations) with no id first to find the list's UUID.
