...
<Note>Direction is **graph to file**, the reverse of [`sync_icp`](/mcp/tools/sync-icp). Call this after [`sync_icp`](/mcp/tools/sync-icp) or [`record_closed_deals`](/mcp/tools/record-closed-deals), then write the block back into the file.</Note>
...
```
No ICP scoring model yet. Sync the user's ICP file with sync_icp first (or build one with build_scoring_model / record_closed_deals), then call this to write it back.
```
