...
- **Nous is the customer graph for GTM agents.** It resolves every person, conversation, and touchpoint across the GTM tool stack (Apollo, HubSpot, Smartlead, Gmail, LinkedIn) into one account record, returned to an agent in a single MCP call.
...
Locked positioning (Nous launch, May 28 2026). Match it in any new or edited page.

- **What Nous is.** "the customer graph for GTM agents." The headline noun is "customer graph." Never "GTM data infrastructure," "context layer," "platform," or "CRM."
...
- **What it ships.** The **account record**, one identity-resolved record of an account, every tool unified, returned in a single MCP call. "Account record" is the product noun underneath the customer graph.
...
- ❌ "one record per human / per person" as the headline unit. The headline noun is the **customer graph**; the product noun is the **account record**. Person-level identity resolution is the *mechanism*, and may be described as such on technical (architecture / API / MCP) pages.
- ❌ "Personalization touchpoint" as marketing copy. Use "touchpoint" alone.
- ❌ Narrowing to a single agent vendor ("built for Claude").

## Terminology

- **customer graph** — the headline noun for the product
- **account record** — the unit Nous returns; the product noun underneath the customer graph
